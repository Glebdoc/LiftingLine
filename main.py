    ...
    def assemble(self, Uinf):
        ...
        for blade in range(self.NB):
            ...
            for i in range(len(self.r_R)-1):
                ...
                vector2 = spatial_transform(shift, np.array([x2[i+1], self.r_R[i+1]*self.R, -z2[i+1]])) + translation 
            ...
            self.collocations = np.hstack((self.collocations, spatial_transform(shift, self.collocations[:,:N], flatten=False)))
    ...
    while (err > 1e-6 and iter<1200):
        ...
        for j in range(NR):
            ...
            for i in range(N*NB):
                ...
                vel1 = np.array([Uinf + u[i+ j*N*NB] + vrot[0], 0 + vrot[1] + v[i+ j*N*NB], 0 + vrot[2] + w[i+ j*N*NB]])
...
if plotting:
    ...
    for rotor in rotors:
        ax.scatter(rotor.collocations[:,0], rotor.collocations[:,1], rotor.collocations[:,2], color='black')
        for horse in rotor.horseShoes:
